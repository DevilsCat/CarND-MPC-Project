...
// Reference speed for speed local optimization.
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Any additons to the cost should be added to 'fg[0]'.
    fg[0] = 0;
    ...
    for (size_t t = 0; t < N; ++t) {
      fg[0] += 5000 * pow(vars[cte_start + t], 2);
      fg[0] += 5000 * pow(vars[epsi_start + t], 2);
      fg[0] += 10 * pow(vars[v_start + t] - ref_v, 2);
    }
    ...
    for (size_t t = 1; t < N; ++t) {
      ...
      fg[1 + x_start + t] = x1 - (x0 + v0 * cos(psi0) * dt);
      fg[1 + y_start + t] = y1 - (y0 + v0 * sin(psi0) * dt);
...
void MPC::Solve(
  ...
  // Set delta's upper and lower limits to -25 and 25 degrees (in radians).
  ...
  for (size_t i = a_start; i < n_vars; ++i) {
    vars_lowerbound[i] = -ACCELERATION_LIMIT;
    vars_upperbound[i] = ACCELERATION_LIMIT;
  }
  ...
  mpc_x_vals = vector<double>(&solution.x[x_start + 1], &solution.x[x_start + N]);
  mpc_y_vals = vector<double>(&solution.x[y_start + 1], &solution.x[y_start + N]);
}
